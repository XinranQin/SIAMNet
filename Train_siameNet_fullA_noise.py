...
parser.add_argument('--end_epoch', type=int, default=402, help='epoch number of end training')
...
Training_data_Name = 'BSDtrain.mat'
Training_data = sio.loadmat('./%s/%s' % (args.data_dir, Training_data_Name))
Training_labels = Training_data['data']
...
Noise_name = './%s/full%d.mat' % (args.data_dir, cs_ratio)
...
def col2im_CS_torch(X_col, row_new, col_new):
    ...
    r_b = row_new // 33
    c_b = col_new // 33
    X_col = X_col.view(-1, r_b, c_b, block_size, block_size)
    X_col = X_col.permute(0, 1, 3, 2, 4)
    X_rec = torch.reshape(X_col, (-1, col_new))
    X_rec = torch.reshape(X_rec, (-1, row_new, col_new))
...
class BasicBlock(torch.nn.Module):
    ...
    def forward(self, x, PhiTPhi, PhiTb, row_new, col_new):
        ...
        x_pred = img2col_torch(x_pred[:, 0], 33)
...
for epoch_i in range(start_epoch, end_epoch + 1):
    for data,n in rand_loader:
        ...
        x_input1 = PhiNx
        x_output1 = model(x_input1, PhiN, Iden, w, h)
...
torch.save(model.state_dict(), "%s/net_params_%d_fullA.pkl" % (model_dir, epoch_i))  # save only the parameters
